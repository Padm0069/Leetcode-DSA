...
public:
    vector<int> shortestAlternatingPaths(int n, vector<vector<int>>& redEdges, vector<vector<int>>& blueEdges) {
        ...
        // Since we can visit all nodes in a graph using BFS traversal, we can update the shortest distance for every node when visiting that particular node. The shortest distance between source node and target node is the distance when we first visit the target node starting from source.
